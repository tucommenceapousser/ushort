...
const argon2 = require('argon2'); // Pour la protection par mot de passe
                        ...
                        ${shortenedURLs.map(({ code, url }) => `
                            <div>
                                <a href="/?c=${code}" target="_blank">
                                    ${process.env.SERVER_URL}/?c=${code} (${url})
                                </a>
...
app.post('/', async (req, res) => {
    ...
    if (originalURL) {
        ...
        shortURL = `${process.env.SERVER_URL}/?c=${shortCode}`;
    }
...
// Endpoint pour supprimer une URL raccourcie, protégé par mot de passe
app.post('/delete', async (req, res) => {
    ...
    if (!codeToDelete || !password) {
        return res.redirect('/?message=Code ou mot de passe manquant');
    }
    ...
    try {
        ...
        if (!isPasswordValid) {
            return res.redirect('/?message=Mot de passe incorrect');
        }
        ...
        res.redirect('/?message=Erreur lors de la suppression');
    }
});

// Endpoint pour le raccourcissement d'URL via API (pour le bot Telegram)
app.post('/api/shorten', async (req, res) => {
    ...
    const shortURL = `${process.env.SERVER_URL}/?c=${shortCode}`;
    res.json({ shortURL });
});
